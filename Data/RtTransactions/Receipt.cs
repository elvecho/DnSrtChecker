...
namespace DnSrtChecker.Data.RtTransactions
{
    public class Receipt
    {
        ...
        {
            get {
                return Turnover.ToString(CultureInfo);
                //return Turnover.ToString().Replace('.', ','); 
            }
            set {
                decimal turnover;
                decimal.TryParse(value, NumberStyles.Any, CultureInfo, out turnover);
                Turnover = turnover;
               // Turnover = decimal.Parse(value.Replace('.', ',')); 
                }
        }
        [XmlIgnore]
        public decimal SumTaxAmountVatTotals { get; set; }
        [XmlElement("Dettagli")]
